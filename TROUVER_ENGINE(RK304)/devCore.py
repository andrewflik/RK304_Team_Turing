"""
    CMFD Model - Taken HELP from Inception v3 Googles Model
    Changed Strcture -> -> Will show in presentation 
    Modified with Custom layers ----> github.com/andewflik978
"""
...
# CUSTOM
def simple_cmfd_decoder( cmdfModel, rgb ) :
    single_sample_batch = np.expand_dims( rgb, axis=0 )
    pred = cmdfModel.predict( single_sample_batch )[0]
    return pred
...
class ResizeBack( Layer ) :
    def call( self, x ) :
        ...
        return tf.compat.v1.image.resize_images( t, new_size, align_corners=True )
...
class Preprocess( Layer ) :
    """
    Basic preprocess layer for CMFD Model
    1) normalize input image size to (256,256)
    2) substract channel-wise means
    """
    def call( self, x, mask=None ) :
        ...
        if (nb_rows != 256) or (nb_cols !=256) :
            ...
            x256 = tf.compat.v1.image.resize_bilinear( x,
                                             [256, 256],
                                             align_corners=True,
                                             name='resize' )
        else :
...
def create_CMFD_model( weight_file=None ) :
    ...
    img_in = Preprocess( name='preprocess')( img_raw )  # YOYOYOYOYO
    ...
    model = Model( inputs = img_raw, outputs = mask_out, name = 'cmdfMOdel')
